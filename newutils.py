...
def get_env_param(optimistic):
    
    # Create a simulation environment
    env = sca.MarketEnvironment(optimistic=optimistic)
...
def get_optimal_vals(lq_time = 60, nm_trades = 60, tr_risk = 1e-6, lambd2 =1e-4,title = ''):
    
    # Create a simulation environment
    env = sca.MarketEnvironment()

    # Reset the enviroment with the given parameters
    env.reset(liquid_time = lq_time, num_trades = nm_trades, lambd1 = tr_risk,lamb2=lambd2)
    print(env.kappa)
...
def plot_trade_list(lq_time = 60, nm_trades = 60, tr_risk = 1e-6,lambd2 = 1e-4,show_trl = False):
    
    # Create simulation environment
    env = sca.MarketEnvironment()

    # Reset the environment with the given parameters
    env.reset(liquid_time = lq_time, num_trades = nm_trades, lamb1 = tr_risk,lamb2=lambd2)
    

    # Get the trading list from the environment
    trade_list = env.get_trade_list(kappa=env.kappa)
...
def get_av_std(lq_time = 60, nm_trades = 60, tr_risk = 1e-6, trs = 100):
    ...
    env.reset(liquid_time = lq_time, num_trades = nm_trades, lamb1 = tr_risk)
