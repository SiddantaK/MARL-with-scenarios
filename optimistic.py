...
import collections
# Common parameters
# Parameters for Optimistic Scenario
ANNUAL_VOLAT = 0.06               # Lower volatility
BID_ASK_SP = 1 / 9              # Narrower bid-ask spread
DAILY_TRADE_VOL = 5e6             # Higher daily trading volume
TRAD_DAYS = 250                   # More trading days in a year
EPSILON = BID_ASK_SP / 2               # Lower fixed cost of selling

DAILY_VOLAT = ANNUAL_VOLAT / np.sqrt(TRAD_DAYS) 
LLAMBDA1 = 1e-6                   # Lower trader's risk aversion for agent 1
LLAMBDA2 = 1e-6                   # Lower trader's risk aversion for agent 2
LIQUIDATION_TIME = 60             # Longer liquidation time
TOTAL_SHARES1 = 550000            # More total shares to sell for agent 1
TOTAL_SHARES2 = 550000            # More total shares to sell for agent 2
STARTING_PRICE = 50               # Higher starting price per share
NUM_N = 60                        # More number of trades
...
class MarketEnvironment():
    ...
    def __init__(self, randomSeed = 0,
                 ...
                 num_tr = NUM_N,
                 lambd1 = LLAMBDA1,
                 lambd2 = LLAMBDA2, optimistic=False):
    ...
    def step(self, action1,action2,rel):
        ...
        if self.transacting1 or self.transacting2:
            ...
            if rel == 'competitive':
                ...
                #reward1 *= 0.5
                    reward1 -= reward2
                    reward1 *= 0.5 
            elif rel == 'cooperative':
                reward1 = (reward1 + reward2)/2
                reward2 = reward1
            else:
                reward1 = 0
                reward2 = 0
